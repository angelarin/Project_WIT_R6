...
class ShirtsListView(generic.ListView):
    ...
    queryset = ProdukItem.objects.filter(kategori='S')
    paginate_by = 4

class SportwearListView(generic.ListView):
    template_name = 'sportwear.html'
    queryset = ProdukItem.objects.filter(kategori='SW')
    paginate_by = 4

class OutwearListView(generic.ListView):
    template_name = 'outwear.html'
    queryset = ProdukItem.objects.filter(kategori='OW')
    paginate_by = 4

class PantsListView(generic.ListView):
    template_name = 'pants.html'
    queryset = ProdukItem.objects.filter(kategori='P')
    paginate_by = 4
    ...
    if request.user.is_authenticated:
        ...
        if order_query.exists():
            ...
            if order.produk_items.filter(produk_item__slug=produk_item.slug).exists():
                order_produk_item.quantity += 100
